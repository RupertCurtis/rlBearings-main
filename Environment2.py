...
class CustomEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        for i in range(self.numberObstacles):
            ...
            self.obstacles.append(pygame.Vector2(x, y))
            self.obstacleBearing[str(pygame.Vector2(x, y))] = getBearing(self.player_pos, pygame.Vector2(x, y), self.heading)
        ...
        while not valid:
            ...
            for obstacle in self.obstacles:
                if getRange(self.finish_pos, obstacle) < 40:
                    clash = True
                    break
                else:
                    clash = False
            if self.finish_pos == self.player_pos:#is finsih same as start
                valid = False
                continue
            elif clash == True:
                valid = False
    ...
    def step(self, action):
        ...
        obstacleReward = 0
        for obstacle in self.obstacles:
            oldBearingContact = self.obstacleBearing[str(obstacle)]
